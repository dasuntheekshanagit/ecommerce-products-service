...
public class CategoryControllerImpl extends AbstractController implements CategoryController {
    ...
    @Override
    public ResponseEntity<ApiResponseDTO<CategoryResponse>> getCategoryById(Long categoryId) {
        log.info("GET /v1/categories/{}", categoryId);
        return ok(() -> categoryService.getCategoryById(categoryId));
    }

    @Override
    public ResponseEntity<ApiResponseDTO<List<CategoryResponse>>> searchCategories(String name) {
